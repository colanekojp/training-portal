...
async function loadDemoGrammarTest(round) {
  const response = await fetch('assets/n2-grammar-demo.json?v=20260922-7');
...
function readGrammarCache(round) {
  try {
    const cached = JSON.parse(localStorage.getItem(`n2-grammar-cache-v3-${round}`));
...
function writeGrammarCache(round, data) {
  try {
    localStorage.setItem(`n2-grammar-cache-v3-${round}`, JSON.stringify({ savedAt: Date.now(), data }));
  } catch (error) {
