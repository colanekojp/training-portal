...
async function loadVocabulary() {
  ...
  try {
    const data = await apiGet('vocabulary', { level: 'N3', unit: state.unit });
...
async function completeVocabulary() {
  ...
  if (!studentId) return setMessage(el['vocabulary-message'], '請輸入正確的 N3 學員編號，例如 N3001。', 'error');
  if (TESTING_MODE) {
...
async function showQuiz() {
  ...
  try {
    const data = await apiGet('quiz', { level: 'N3', unit: state.unit });
...
async function submitQuiz(event) {
  ...
  if (!studentId) return setMessage(el['quiz-message'], '請輸入正確的 N3 學員編號，例如 N3001。', 'error');
  const answers = {};
  ...
  try {
    ...
    const result = await apiPost({ action: 'submit_quiz', level: 'N3', student_id: studentId, unit: state.unit, answers });
