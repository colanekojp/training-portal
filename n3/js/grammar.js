...
async function loadDemoGrammarTest(round) {
  const response = await fetch('assets/n3-grammar-demo.json?v=20260920-4');
...
function readGrammarCache(round) {
  try {
    const cached = JSON.parse(localStorage.getItem(`n3-grammar-cache-v1-${round}`));
...
function writeGrammarCache(round, data) {
  try {
    localStorage.setItem(`n3-grammar-cache-v1-${round}`, JSON.stringify({ savedAt: Date.now(), data }));
  } catch (error) {
...
function renderGrammarTest() {
  ...
  document.title = `${data.title || `N3 文法測驗第 ${state.round} 回`}｜2026第2回JLPT特訓班`;
...
async function submitGrammarTest(event) {
  ...
  if (!studentId) {
    setMessage('請輸入正確的 N3 學員編號，例如 N3001。', 'error');
    el['student-id'].focus();
    return;
  }
...
function showDemoResult() {
  el['grammar-result'].innerHTML = `
    ...
    <p>正式模式會由 GAS 計算答對題數、保存每題作答，並將 N3 點名表的「文法${state.round}」打勾。</p>
