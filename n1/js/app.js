...
const PORTAL_CACHE_KEY = 'n1-portal-cache-v3';
...
function createFallbackPortal() {
  ...
  return {
    level: LEVEL,
    pageTitle: 'N1學習專區｜2026第2回JLPT特訓班',
    announcement: '',
...
async function submitListeningForm(event) {
  ...
  if (!studentId) return setListeningMessage('請輸入正確的 N1 學員編號，例如 N1001。', 'error');
...
function normalizeStudentId(value) {
  ...
  return /^N1\d{3}$/.test(normalized) ? normalized : '';
}
