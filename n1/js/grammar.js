...
async function loadDemoGrammarTest(round) {
  const response = await fetch('assets/n1-grammar-demo.json?v=20260920-4');
...
function readGrammarCache(round) {
  try {
    const cached = JSON.parse(localStorage.getItem(`n1-grammar-cache-v1-${round}`));
...
function writeGrammarCache(round, data) {
  try {
    localStorage.setItem(`n1-grammar-cache-v1-${round}`, JSON.stringify({ savedAt: Date.now(), data }));
  } catch (error) {
