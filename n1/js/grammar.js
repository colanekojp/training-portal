...
async function loadDemoGrammarTest(round) {
  const response = await fetch('assets/n1-grammar-demo.json?v=20260922-6');
...
function readGrammarCache(round) {
  try {
    const cached = JSON.parse(localStorage.getItem(`n1-grammar-cache-v2-${round}`));
...
function writeGrammarCache(round, data) {
  try {
    localStorage.setItem(`n1-grammar-cache-v2-${round}`, JSON.stringify({ savedAt: Date.now(), data }));
  } catch (error) {
...
function renderGrammarTest() {
  ...
  document.title = `${data.title || `N1 文法測驗第 ${state.round} 回`}｜2026第2回JLPT特訓班`;
...
async function submitGrammarTest(event) {
  ...
  if (!studentId) {
    setMessage('請輸入正確的 N1 學員編號，例如 N1001。', 'error');
    el['student-id'].focus();
    return;
  }
...
function showDemoResult() {
  el['grammar-result'].innerHTML = `
    ...
    <p>正式模式會由 GAS 計算答對題數、保存每題作答，並將 N1 點名表的「文法${state.round}」打勾。</p>
...
function showResult(data, message) {
  ...
  el['grammar-result'].innerHTML = `
    ...
    <div class="grammar-result-actions">
      <a class="button button-secondary" href="index.html">返回 N1 學習專區</a>
