...
async function loadVocabulary() {
  ...
  try {
    const data = await apiGet('vocabulary', { level: 'N1', unit: state.unit });
...
async function completeVocabulary() {
  ...
  if (!studentId) return setMessage(el['vocabulary-message'], '請輸入正確的 N1 學員編號，例如 N1001。', 'error');
  if (TESTING_MODE) {
...
async function showQuiz() {
  ...
  try {
    const data = await apiGet('quiz', { level: 'N1', unit: state.unit });
...
function readLearningCache(type, unit) {
  try {
    const cached = JSON.parse(localStorage.getItem(`n1-${type}-cache-v2-${unit}`));
...
function writeLearningCache(type, unit, data) {
  try {
    localStorage.setItem(`n1-${type}-cache-v2-${unit}`, JSON.stringify({ savedAt: Date.now(), data }));
  } catch (error) {
...
async function submitQuiz(event) {
  ...
  if (!studentId) return setMessage(el['quiz-message'], '請輸入正確的 N1 學員編號，例如 N1001。', 'error');
  const answers = {};
